...
class ResultsScreen extends React.Component {
  ...
  render() {
    ...
    for (let i = 0; i < this.props.product.length; i++) {
      totalcp =
        totalcp +
        Number(this.props.product[i].cp) * Number(this.props.product[i].sold);
      // console.log((Number(this.props.product[i].cp)*Number(this.props.product[i].sold)))
      console.log(totalcp);
      totalsp =
        totalsp +
        Number(this.props.product[i].sp) * Number(this.props.product[i].sold);
      // console.log((Number(this.props.product[i].sp)*Number(this.props.product[i].sold)))
      console.log(totalsp);
    }

    for (let i = 0; i < this.props.pastries.length; i++) {
      pastriesTotalSalescp =
        pastriesTotalSalescp +
        Number(this.props.pastries[i].cp) * Number(this.props.pastries[i].sold);
      pastriesTotalSalessp =
        pastriesTotalSalessp +
        Number(this.props.pastries[i].sp) * Number(this.props.pastries[i].sold);
    }
      ...
      <View style={styles.container}>
        <View style={styles.structuralCategorySection}>
          ...
          <View style={styles.vendorAgentPayContainer}>
            ...
            <Text style={styles.amtInCedisText}>
              GHS {(totalsp - totalcp).toFixed(2)}
            </Text>
          </View>
          <View style={styles.vendorAgentPayContainer}>
        ...
        <View style={styles.structuralCategorySection}>
          ...
          <View style={styles.vendorAgentPayContainer}>
            <Text style={styles.paySectionText}>Amount due Vendor:</Text>
            <Text style={styles.amtInCedisText}>
              GHS {(0.3 * pastriesTotalSalessp).toFixed(2)}
            </Text>
          </View>
          <View style={styles.vendorAgentPayContainer}>
            <Text style={styles.paySectionText}>Amount due Agent:</Text>
            <Text style={styles.amtInCedisText}>
              GHS {(0.7 * pastriesTotalSalessp).toFixed(2)}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.structuralCategorySection}>
          ...
          <View style={styles.vendorAgentPayContainer}>
            <Text style={styles.paySectionText}>Amount due Vendor:</Text>
            <Text style={styles.amtInCedisText}>
              GHS {(totalsp - totalcp + 0.3 * pastriesTotalSalessp).toFixed(2)}{' '}
            </Text>
          </View>
          <View style={styles.vendorAgentPayContainer}>
            <Text style={styles.paySectionText}>Amount due Agent:</Text>
            <Text style={styles.amtInCedisText}>
              GHS {(totalcp + 0.7 * pastriesTotalSalessp).toFixed(2)}
            </Text>
          </View>
        </View>

        <View style={styles.okayButtonContainer}></View>
      </View>
    );
  }
}
